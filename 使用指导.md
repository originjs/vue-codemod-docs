...
- [使用指引](#使用指引)
  - [安装](#安装)
    - [`npm/yarn` 方式](#npmyarn-方式)
    - [`clone` 安装方式](#clone-安装方式)
  - [使用](#使用)
    - [执行所有规则](#执行所有规则)
    - [执行单条规则](#执行单条规则)
    - [规定输出格式](#规定输出格式)
    - [帮助](#帮助)
- [<span id='ruleList'>规则清单</span>](#规则清单)
## 介绍
...
选择 `detail` 时会以对象形式输出具体修改的规则，选择 `table` 时则会以表格的形式输出修改的规则，而选择 `log` 则会将统计报告以 log 文件的形式输出。
